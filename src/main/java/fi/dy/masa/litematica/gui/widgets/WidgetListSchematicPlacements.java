...
public class WidgetListSchematicPlacements
extends WidgetListBase<SchematicPlacement, WidgetSchematicPlacement> {
    ...
    protected List<String> getEntryStringsForFilter(SchematicPlacement entry) {
        if (entry.getSchematic().getFile() != null) {
            String fileName = FileNameUtils.getFileNameWithoutExtension((String)entry.getSchematic().getFile().getName().toLowerCase());
            return ImmutableList.of((Object)entry.getName().toLowerCase(), (Object)fileName);
        }
    ...
    protected WidgetSchematicPlacement createListEntryWidget(int x, int y, int listIndex, boolean isOdd, SchematicPlacement entry) {
        return new WidgetSchematicPlacement(x, y, this.browserEntryWidth, this.getBrowserEntryHeightFor(entry), isOdd, entry, listIndex, this);
    }
}
